...
static RPCHelpMan debug()
{
    return RPCHelpMan{"debug",
        ...
        " - \"orin\" activates all Orin-specific categories at once;\n"
        ...
        RPCExamples {
            HelpExampleCli("debug", "orin")
    + HelpExampleRpc("debug", "orin+net")
        },
...
static RPCHelpMan logging()
{
    ...
    ,
        ...
        RPCExamples{
          ...
          + HelpExampleCli("logging", "'[\"orin\"]' '[\"llmq\",\"zmq\"]'")
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "orin-cli and the GUI. There is no server-side difference.",
                {
                ...
                [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (g_mappoint_index) {
        result.pushKVs(SummaryToJSON(g_mappoint_index->GetSummary(), index_name));
    }
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                auto init = ipc->spawnProcess("orin-node");
                echo = init->makeEcho();
