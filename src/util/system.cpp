...
const char * const BITCOIN_CONF_FILENAME = "orin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //orin-tx using stdin
        ...
        if (key[0] != '-') {
            if (!m_accept_any_command && m_command.empty()) {
                // The first non-orin arg is a registered command
...
{
    // Windows: C:\Users\Username\AppData\Roaming\OrinCore
    // macOS: ~/Library/Application Support/OrinCore
    // Unix-like: ~/.orincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OrinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/OrinCore";
#else
    // Unix-like
    return pathRet / ".orincore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    std::ifstream stream = fsbridge::ifstream(conf_path);
    ...
    if (stream.good()) {
        ...
        if (use_conf_file) {
            ...
            for (const std::string& conf_file_name : conf_file_names) {
                std::ifstream conf_file_stream = fsbridge::ifstream(GetConfigFile(fs::PathFromString(conf_file_name)));
        ...
        // Create an empty orin.conf if it does not exist
        const fs::path config_file{GetConfigFile(conf_path)};
        ...
        if (!config_dir.empty() && !fs::exists(config_dir)) {
            try {
                ...
                error = strprintf("could not create configuration directory \"%s\": %s", fs::PathToString(config_dir), e.what());
                return false;
            }
        }
        std::ofstream configFile = fsbridge::ofstream(config_file, std::ios_base::app);
        if (!configFile.good()) {
            error = strprintf("could not create configuration file \"%s\".", fs::PathToString(config_file));
            return false;
        }
        configFile.close();
