...
impl PrivateKey {
    ...
    #[cfg(feature = "orin_helpers")]
    pub fn generate_orin(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "orin_helpers")]
    pub fn generate_orin_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_orin(rng))
            .collect()
    }
