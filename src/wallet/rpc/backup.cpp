...
RPCHelpMan importaddress()
{
    ...
    {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Orin address (or hex-encoded script)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Orin address or script");
        }
    }
    if (fRescan)
    {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        std::ifstream file = fsbridge::ifstream(fs::u8path(request.params[0].get_str()), std::ios::in | std::ios::ate);
        if (!file.is_open()) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::ifstream file = fsbridge::ifstream(fs::PathFromString(strFileName), std::ios::in | std::ios::ate);
    if (!file.is_open())
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
        ...
        "Note: This command is only compatible with legacy wallets.\n",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Orin address for the private key"},
        },
        RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::ofstream file = fsbridge::ofstream(filepath);
    if (!file.is_open())
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot open wallet dump file");
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("orincoreversion", FormatFullVersion());
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see orind log for details) and could "
