...
namespace fsbridge {
    ...
    inline std::ifstream ifstream(const fs::path& p, std::ios_base::openmode mode = std::ios_base::in)
    {
        return std::ifstream{std::filesystem::path{p}, mode};
    }
    inline std::ofstream ofstream(const fs::path& p, std::ios_base::openmode mode = std::ios_base::out)
    {
        return std::ofstream{std::filesystem::path{p}, mode};
    }
