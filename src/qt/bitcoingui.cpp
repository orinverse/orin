...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Orin address"));
    ...
    coinJoinCoinsAction->setStatusTip(tr("Send %1 funds to a Orin address").arg(strCoinJoinName));
    coinJoinCoinsAction->setToolTip(coinJoinCoinsAction->statusTip());

    mapPointsAction = new QAction(tr("&Map Points"), this);
    mapPointsAction->setStatusTip(tr("Create and view RealMap points stored on-chain"));
    mapPointsAction->setToolTip(mapPointsAction->statusTip());

    receiveCoinsAction = new QAction(tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and orin: URIs)"));
    ...
    connect(mapPointsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(mapPointsAction, &QAction::triggered, this, &BitcoinGUI::gotoMapPointsPage);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Orin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Orin addresses"));
    ...
    openAction->setStatusTip(tr("Open a orin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Orin command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        mapPointsButton = new QToolButton(this);
        mapPointsButton->setText(mapPointsAction->text());
        mapPointsButton->setStatusTip(mapPointsAction->statusTip());
        tabGroup->addButton(mapPointsButton);
        ...
        connect(mapPointsButton, &QToolButton::clicked, this, &BitcoinGUI::gotoMapPointsPage);
        ...
        m_wallet_selector_action = appToolBar->addWidget(walletSelector);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    if (walletFrame != nullptr) {
        ...
        if (mapPointsButton) mapPointsButton->setEnabled(enabled);
...
#endif // ENABLE_WALLET
    if (mapPointsAction) mapPointsAction->setEnabled(enabled);
...
void BitcoinGUI::gotoMapPointsPage()
{
    if (mapPointsButton) {
        mapPointsButton->setChecked(true);
    }
    if (walletFrame) walletFrame->gotoMapPointsPage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        ...
        tooltip = tr("%n active connection(s) to Orin network", "", count);
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the orin logo
