...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def orin_wallet_process(self, *args):
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.orin_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `orin-wallet help` raises an error. Use `orin-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `orin-wallet -help` for valid methods.')
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of orin-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        ...
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of orin-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
