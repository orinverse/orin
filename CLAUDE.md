...
Orin Core is the reference implementation for Orin, a cryptocurrency. It builds on top of Bitcoin Core, a codebase that
is primarily written in C++20 (requiring at least Clang 16 or GCC 11.1). Orin Core uses the GNU Autotools build system.
...
- **Implementation** `src/` - C++20 codebase
  ...
  - `src/qt/` - Implementation of Orin Qt, the GUI (uses Qt 5)
...
- **Functional Tests**: `test/functional/` - Python tests (minimum version in `.python-version`) dependent on `orind` and `orin-node`
...
- Vendored dependencies:
  - `src/{crc32c,orinbls,gsl,immer,leveldb,minisketch,secp256k1,univalue}`
...
./src/test/test_orin --run_test=getarg_tests

# Debug unit tests
gdb ./src/test/test_orin
```
...
Orin Core extends Bitcoin Core through composition, using a layered architecture:

```
Orin Core Components
...
#### Orin-Specific Databases

- **CFlatDB**: A Orin-specific flat file database format used for persistent storage
...
- **CDBWrapper**: Bitcoin Core database wrapper extended for Orin-specific data
...
2. **Parameter Interaction**: Orin-specific configuration validation
3. **Interface Setup**: Orin manager instantiation in NodeContext
...
- **ChainstateManager**: Enhanced with Orin-specific validation
...
# Run orind with debug logging
./src/orind -debug=all -printtoconsole

# Run functional test with custom orind
test/functional/test_runner.py --orind=/path/to/orind
...
# Memory debugging
valgrind --leak-check=full ./src/orind
```
...
gh api repos/orinpay/orin/actions/jobs/<JOB_ID>/logs
...
# gh api repos/orinpay/orin/actions/jobs/46274126203/logs
```
...
- Orin uses `unordered_lru_cache` for efficient caching with LRU eviction
- The codebase extensively uses Orin-specific data structures for performance
