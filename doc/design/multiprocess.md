...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `orin-node`, `orin-wallet`, and `orin-gui` executables alongside existing `orind` and `orin-qt` executables.

`orin-node` is a drop-in replacement for `orind`, and `orin-gui` is a drop-in replacement for `orin-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `orin-gui` will spawn a `orin-node` process to run P2P and RPC code, communicating with it across a socket pair, and `orin-node` will spawn `orin-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `orin-node`, `orin-wallet`, and `orin-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `orin-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
make
src/orin-node -regtest -printtoconsole -debug=ipc
DASHD=orin-node test/functional/test_runner.py
```
