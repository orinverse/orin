...
Currently, 10% of block rewards are set aside for the Orin DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Orin in a
credit pool. This credit pool mirrors the available credit supply in Orin
Platform. When a certain amount of Orin is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Orin
Platform. Masternode identities receive the fees and can convert them back to
Orin by withdrawing them from Orin Platform.
...
### Removal of reject network messages from Orin Core (BIP61)

This feature has been disabled by default since Orin Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Orin Core version 20.0.0 there are extra changes:
...
- It is possible to run Orin Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `ORIN/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Orin Core (BIP61)_](#removal-of-reject-network-messages-from-orin-core-bip61) above for
...
### Orin-specific changes in existing RPCs:
...
- The `orin-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `orind --help` for
more information.
...
python test framework and `orin-cli` tool both take advantage of this, so
for example:

```sh
orin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
orin-cli -named createwallet mywallet load_on_startup=1
```
...
with Orin’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/orinpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/orinpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/orinpay/orin/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/orinpay/orin/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/orinpay/orin/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/orinpay/orin/compare/v19.3.0...orinpay:v20.0.0
