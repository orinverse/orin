# CJDNS support in Orin Core

It is possible to run Orin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Orin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Orin network and individual nodes.
...
before it will work with your Orin Core node. This is described in steps
...
## Run Orin Core with CJDNS

Once you are connected to the CJDNS network, the following Orin Core
...
When enabled, this option tells Orin Core that it is running in an
...
IPv6 local network. This helps Orin Core perform better address management:
...
CJDNS support was added to Orin Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `orin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/orinpay/orin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Orin Core

There are several ways to see your CJDNS address in Orin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `orin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `orin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `orin-cli help getnodeaddresses` for details.
