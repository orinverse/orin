...
RUN set -ex; \
    cd /tmp; \
    git clone --depth 1 --no-tags --branch=${DASH_HASH_VERSION} https://github.com/orinpay/orin_hash; \
    cd orin_hash && uv pip install --system --break-system-packages -r requirements.txt .; \
    cd .. && rm -rf orin_hash
...
ARG USER_ID=1000 \
    GROUP_ID=1000
RUN set -ex; \
    getent group ${GROUP_ID} || groupmod -g ${GROUP_ID} -n orin ubuntu; \
    usermod -u ${USER_ID} -md /home/orin -l orin ubuntu; \
    chown ${USER_ID}:${GROUP_ID} -R /home/orin; \
    mkdir -p /src/orin && \
